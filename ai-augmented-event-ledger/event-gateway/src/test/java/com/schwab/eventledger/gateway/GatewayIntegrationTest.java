...
class GatewayIntegrationTest {
    ...
    void acceptsEventsIdempotentlyListsChronologicallyAndPropagatesTraceIds() {
        ...
        var auditEntries = restTemplate.getForObject("/events/gw-evt-001/audit", AuditEntryResponse[].class);

        assertThat(auditEntries).extracting(AuditEntryResponse::action)
                .containsExactly("EVENT_ACCEPTED", "ACCOUNT_APPLY_SUCCEEDED", "DUPLICATE_SUBMISSION");
        assertThat(auditEntries).extracting(AuditEntryResponse::traceId)
                .contains("trace-gateway-456", "trace-gateway-789");
    }
